...
%legg_til_datasett(dsn=ks_akuttp0_ijust_bohf, original=datasett, first = 1, utv=ks_akutt);
%legg_til_datasett(dsn=tang_vakuump0_ijust_bohf, original=datasett, utv=tang_vak);
%legg_til_datasett(dsn=blod15p0vag_ijust_bohf, original=datasett, utv=blod15);
%legg_til_datasett(dsn=rupturp0_ijust_bohf, original=datasett, utv=ruptur);
%legg_til_datasett(dsn=apgar7p0_ijust_bohf, original=datasett, utv=apgar7);
...
ODS Graphics ON / imagemap reset=All imagename="scatter_inngr_kompl_p0" imagefmt=png  border=off width=14.0cm height=10.0cm;
...
scatter x=x y=faktori / jitter markerattrs=(SYMBOL=circleFilled color=blue) legendlabel="Store inngrep";
...
rowaxis display=(nolabel ) values=(0.5 to 1.5 by 0.2);
run;
ods listing close;
ods graphics off; 
