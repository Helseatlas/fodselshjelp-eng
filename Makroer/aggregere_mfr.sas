...
data FH_&tema._&par_rob.&grp.;
  ...
  *keep the desired group, i.e. par0, par1, rob1, rob2, rob3, rob4, or rob9;
  ...
  %else %if &nevner=alle %then %do;
    %if                  &grp=90 %then %do;                 *Angir robson gr 9 for par 0 som 90;
      if aldersgruppe in (1,2,3) and robson = 9 then output;    *Velger ut aktuell gruppe med krav til både robson-variabel og aldersgruppe for å skille par0 og par 1;
    %end;
    %else %if                  &grp=91 %then %do;                 *Angir robson gr 9 for par 1 som 91;
      if aldersgruppe in (4,5,6) and robson = 9 then output;    *Velger ut aktuell gruppe med krav til både robson-variabel og aldersgruppe for å skille par0 og par 1;
    %end;
    %else %do;
